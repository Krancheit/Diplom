...
private: // ---------------------------------------------------------------
  ...
  McSpectrDataFreq rFreqHi;    // parameters, modified in Freq. Settings
  McFreqApply *prApp;
...
void __fastcall trMcSpectrTrendR::SettingsClick(TObject *Sender)
{
  if( bInitialized )
  {
    ...
    bool bShowMGA = bMGA;
	McEditSpectrTrendSettings(&rSP, &rTB, (trMcApplyObj *)(&rAppSet),
	 &bShowView, &bShowMGA);  // sets everything by itself
  }
} // ***
...
void __fastcall TFrameSpectrTrend::MenuItemFreqClick(TObject *Sender)
{
	McEditSetFreq();
}
//---------------------------------------------------------------------------
